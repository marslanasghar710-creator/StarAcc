...
class BankTransaction(Base, UUIDPKMixin, TimestampMixin):
    ...
    transaction_type: Mapped[BankTransactionType] = mapped_column(Enum(BankTransactionType, name="bank_transaction_type", values_callable=lambda enum_cls: [item.value for item in enum_cls]), nullable=False)
    amount: Mapped[Decimal] = mapped_column(Numeric(20, 8), nullable=False)
    description: Mapped[str] = mapped_column(String(500), nullable=False)
    reference: Mapped[str | None] = mapped_column(String(100), nullable=True)
    memo: Mapped[str | None] = mapped_column(Text, nullable=True)
    status: Mapped[BankTransactionStatus] = mapped_column(Enum(BankTransactionStatus, name="bank_transaction_status", values_callable=lambda enum_cls: [item.value for item in enum_cls]), nullable=False, default=BankTransactionStatus.UNRECONCILED)
