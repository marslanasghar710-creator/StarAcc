...
class AutomationRule(Base, UUIDPKMixin, TimestampMixin):
    ...
    rule_type: Mapped[AutomationRuleType] = mapped_column(Enum(AutomationRuleType, name="automationruletype", values_callable=lambda enum_cls: [item.value for item in enum_cls]), nullable=False)
...
class Suggestion(Base, UUIDPKMixin, TimestampMixin):
    ...
    status: Mapped[SuggestionStatus] = mapped_column(Enum(SuggestionStatus, name="suggestionstatus", values_callable=lambda enum_cls: [item.value for item in enum_cls]), nullable=False, default=SuggestionStatus.PENDING, index=True)
    ...
    source_type: Mapped[SuggestionSourceType] = mapped_column(Enum(SuggestionSourceType, name="suggestionsourcetype", values_callable=lambda enum_cls: [item.value for item in enum_cls]), nullable=False)
...
class SuggestionFeedback(Base, UUIDPKMixin, TimestampMixin):
    ...
    action: Mapped[SuggestionFeedbackAction] = mapped_column(Enum(SuggestionFeedbackAction, name="suggestionfeedbackaction", values_callable=lambda enum_cls: [item.value for item in enum_cls]), nullable=False)
...
class DocumentExtractionJob(Base, UUIDPKMixin, TimestampMixin):
    ...
    status: Mapped[DocumentExtractionStatus] = mapped_column(Enum(DocumentExtractionStatus, name="documentextractionstatus", values_callable=lambda enum_cls: [item.value for item in enum_cls]), nullable=False, default=DocumentExtractionStatus.PENDING, index=True)
    classifier_label: Mapped[DocumentClassifierLabel | None] = mapped_column(Enum(DocumentClassifierLabel, name="documentclassifierlabel", values_callable=lambda enum_cls: [item.value for item in enum_cls]), nullable=True)
...
class ReconciliationSuggestionSet(Base, UUIDPKMixin, TimestampMixin):
    ...
    generation_status: Mapped[AIJobStatus] = mapped_column(Enum(AIJobStatus, name="aijobstatus", values_callable=lambda enum_cls: [item.value for item in enum_cls]), nullable=False, default=AIJobStatus.PENDING)
...
class AIProcessingJob(Base, UUIDPKMixin, TimestampMixin):
    ...
    organization_id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), ForeignKey("organizations.id"), nullable=False, index=True)
    job_type: Mapped[AIJobType] = mapped_column(Enum(AIJobType, name="aijobtype", values_callable=lambda enum_cls: [item.value for item in enum_cls]), nullable=False)
    target_entity_type: Mapped[str] = mapped_column(String(50), nullable=False, index=True)
    target_entity_id: Mapped[str] = mapped_column(String(100), nullable=False, index=True)
    status: Mapped[AIJobStatus] = mapped_column(Enum(AIJobStatus, name="aijobstatus", create_type=False, values_callable=lambda enum_cls: [item.value for item in enum_cls]), nullable=False, default=AIJobStatus.PENDING, index=True)
