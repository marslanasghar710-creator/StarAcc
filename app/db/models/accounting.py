...
class Account(Base, UUIDPKMixin, TimestampMixin):
    ...
    account_type: Mapped[AccountType] = mapped_column(Enum(AccountType, name="account_type", values_callable=lambda enum_cls: [item.value for item in enum_cls]), nullable=False)
    account_subtype: Mapped[str | None] = mapped_column(String(100), nullable=True)
    normal_balance: Mapped[NormalBalance] = mapped_column(Enum(NormalBalance, name="normal_balance", values_callable=lambda enum_cls: [item.value for item in enum_cls]), nullable=False)
...
class FinancialPeriod(Base, UUIDPKMixin, TimestampMixin):
    ...
    status: Mapped[PeriodStatus] = mapped_column(Enum(PeriodStatus, name="period_status", values_callable=lambda enum_cls: [item.value for item in enum_cls]), nullable=False, default=PeriodStatus.OPEN)
...
class JournalEntry(Base, UUIDPKMixin, TimestampMixin):
    ...
    status: Mapped[JournalStatus] = mapped_column(Enum(JournalStatus, name="journal_status", values_callable=lambda enum_cls: [item.value for item in enum_cls]), nullable=False, default=JournalStatus.DRAFT)
