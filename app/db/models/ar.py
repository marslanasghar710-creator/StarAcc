...
class Invoice(Base, UUIDPKMixin, TimestampMixin):
    ...
    status: Mapped[InvoiceStatus] = mapped_column(Enum(InvoiceStatus, name="invoice_status", values_callable=lambda enum_cls: [item.value for item in enum_cls]), nullable=False, default=InvoiceStatus.DRAFT)
    invoice_type: Mapped[InvoiceType] = mapped_column(Enum(InvoiceType, name="invoice_type", values_callable=lambda enum_cls: [item.value for item in enum_cls]), nullable=False, default=InvoiceType.STANDARD)
...
class InvoiceItem(Base, UUIDPKMixin, TimestampMixin):
    ...
    item_id: Mapped[uuid.UUID | None] = mapped_column(UUID(as_uuid=True), ForeignKey("items.id"), nullable=True, index=True)
...
class CreditNote(Base, UUIDPKMixin, TimestampMixin):
    ...
    status: Mapped[CreditNoteStatus] = mapped_column(Enum(CreditNoteStatus, name="credit_note_status", values_callable=lambda enum_cls: [item.value for item in enum_cls]), nullable=False, default=CreditNoteStatus.DRAFT)
...
class CustomerPayment(Base, UUIDPKMixin, TimestampMixin):
    ...
    status: Mapped[PaymentStatus] = mapped_column(Enum(PaymentStatus, name="payment_status", values_callable=lambda enum_cls: [item.value for item in enum_cls]), nullable=False, default=PaymentStatus.DRAFT)
