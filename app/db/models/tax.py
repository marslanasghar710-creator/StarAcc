...
class TaxSettings(Base, UUIDPKMixin, TimestampMixin):
    ...
    tax_basis: Mapped[TaxBasis] = mapped_column(Enum(TaxBasis, name="tax_basis", values_callable=lambda enum_cls: [item.value for item in enum_cls]), nullable=False, default=TaxBasis.ACCRUAL)
    prices_entered_are: Mapped[PricesEnteredAre] = mapped_column(Enum(PricesEnteredAre, name="prices_entered_are", values_callable=lambda enum_cls: [item.value for item in enum_cls]), nullable=False, default=PricesEnteredAre.EXCLUSIVE)
    ...
    tax_rounding_method: Mapped[TaxRoundingMethod] = mapped_column(Enum(TaxRoundingMethod, name="tax_rounding_method", values_callable=lambda enum_cls: [item.value for item in enum_cls]), nullable=False, default=TaxRoundingMethod.LINE)
    tax_reporting_currency: Mapped[str | None] = mapped_column(String(3), nullable=True)
    tax_periodicity: Mapped[TaxPeriodicity | None] = mapped_column(Enum(TaxPeriodicity, name="tax_periodicity", values_callable=lambda enum_cls: [item.value for item in enum_cls]), nullable=True)
...
class TaxRate(Base, UUIDPKMixin, TimestampMixin):
    ...
    tax_type: Mapped[TaxType] = mapped_column(Enum(TaxType, name="tax_type", values_callable=lambda enum_cls: [item.value for item in enum_cls]), nullable=False)
    scope: Mapped[TaxScope] = mapped_column(Enum(TaxScope, name="tax_scope", values_callable=lambda enum_cls: [item.value for item in enum_cls]), nullable=False, default=TaxScope.BOTH)
...
class TaxCode(Base, UUIDPKMixin, TimestampMixin):
    ...
    description: Mapped[str | None] = mapped_column(String(255), nullable=True)
    is_active: Mapped[bool] = mapped_column(Boolean, nullable=False, default=True)
    applies_to: Mapped[TaxCodeAppliesTo] = mapped_column(Enum(TaxCodeAppliesTo, name="tax_code_applies_to", values_callable=lambda enum_cls: [item.value for item in enum_cls]), nullable=False, default=TaxCodeAppliesTo.BOTH)
    calculation_method: Mapped[TaxCalculationMethod] = mapped_column(Enum(TaxCalculationMethod, name="tax_calculation_method", values_callable=lambda enum_cls: [item.value for item in enum_cls]), nullable=False)
    price_inclusive_behavior: Mapped[TaxPriceInclusiveBehavior] = mapped_column(Enum(TaxPriceInclusiveBehavior, name="tax_price_inclusive_behavior", values_callable=lambda enum_cls: [item.value for item in enum_cls]), nullable=False, default=TaxPriceInclusiveBehavior.INHERIT_ORGANIZATION_DEFAULT)
    report_group: Mapped[str | None] = mapped_column(String(100), nullable=True)
...
class TaxTransaction(Base, UUIDPKMixin):
    ...
    direction: Mapped[TaxTransactionDirection] = mapped_column(Enum(TaxTransactionDirection, name="tax_transaction_direction", values_callable=lambda enum_cls: [item.value for item in enum_cls]), nullable=False, index=True)
