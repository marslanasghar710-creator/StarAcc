...
class ConsolidationRepository:
    ...
    def list_groups(self, organization_id: str | UUID) -> list[ConsolidationGroup]:
        return list(
            self.db.scalars(
    ...
    def list_runs(self, group_id: str | UUID) -> list[ConsolidationRun]:
        return list(
            self.db.scalars(
                select(ConsolidationRun).where(ConsolidationRun.group_id == group_id).order_by(ConsolidationRun.created_at.desc())
            ).all()
        )
    ...
    def list_elimination_entries(self, group_id: str | UUID, *, period_start: date | None = None, period_end: date | None = None) -> list[EliminationEntry]:
