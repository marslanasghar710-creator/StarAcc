...
import { getInvoiceDraftPreview, type InvoiceFormValues } from "@/features/invoices/schemas";
...
export function InvoiceLineItemsEditor({
  ...
  const preview = getInvoiceDraftPreview(items);
    ...
    <div className="overflow-hidden rounded-xl border border-border/70 bg-card shadow-xs">
        ...
        <table className="w-full min-w-[1100px] text-sm">
          <thead className="bg-muted/40 text-muted-foreground">
            <tr>
              ...
              <th className="px-3 py-2 text-left font-medium">Revenue account</th>
          ...
          <tbody>
            {fieldArray.fields.map((field, index) => {
              const linePreview = getInvoiceDraftPreview([{ quantity: items[index]?.quantity, unit_price: items[index]?.unit_price }]);
              return (
                <tr key={field.id} className="border-t border-border/60 align-top">
                  ...
                  <td className="px-3 py-3">
                    <FormField
                      ...
                      render={({ field: currentField }) => (
                        <FormItem>
                          <FormControl>
                            <Input {...currentField} value={currentField.value ?? ""} disabled={readOnly} placeholder="Service or item description" />
                          </FormControl>
                          <FormMessage />
                        </FormItem>
                      )}
                  ...
                  <td className="px-3 py-3">
                    <FormField
                      ...
                      render={({ field: currentField }) => (
                        <FormItem>
                          <FormControl>
                            <Input {...currentField} value={currentField.value ?? ""} disabled={readOnly} placeholder="Optional SKU" />
                          </FormControl>
                          <FormMessage />
                        </FormItem>
                      )}
                  ...
                  <td className="px-3 py-3">
                    <FormField
                      ...
                      render={({ field: currentField }) => (
                        <FormItem>
                          <FormControl>
                            <Input {...currentField} value={currentField.value ?? ""} disabled={readOnly} inputMode="decimal" className="text-right tabular-nums" />
                          </FormControl>
                          <FormMessage />
                        </FormItem>
                      )}
                  ...
                  <td className="px-3 py-3 text-right font-medium"><MoneyDisplay value={linePreview.previewSubtotal} currencyCode={currencyCode} /></td>
                  <td className="px-3 py-3 text-right">
                    <Button type="button" variant="ghost" size="icon" aria-label={`Remove item ${index + 1}`} disabled={readOnly || fieldArray.fields.length <= 1} onClick={() => fieldArray.remove(index)}>
      ...
      <div className="flex items-center justify-between border-t border-border/60 bg-muted/20 px-4 py-3">
        <div className="space-y-1 text-sm text-muted-foreground">
          <p>Client preview only. Taxes, discounts, and posted totals remain backend authoritative.</p>
