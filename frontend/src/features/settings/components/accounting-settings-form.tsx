    ...
    <Form {...form}>
      <form className="space-y-4" onSubmit={form.handleSubmit(handleSubmit)}>
        ...
        <div className="grid gap-4 md:grid-cols-2">
          {(["default_locale", "date_format", "number_format", "base_currency", "timezone"] as const).map((name) => (
        ...
        {(["tax_enabled", "multi_currency_enabled"] as const).map((name) => (
          <FormField key={name} control={form.control} name={name} render={({ field }) => (<FormItem className="rounded-lg border border-border/70 p-3"><FormLabel className="flex items-center justify-between"><span>{name.replaceAll("_", " ")}</span><input type="checkbox" checked={field.value} onChange={(event) => field.onChange(event.target.checked)} disabled={readOnly || isSubmitting} /></FormLabel><FormMessage /></FormItem>)} />
        ))}
